...
def load_data(path="Secadero 1 automático.xlsx"):
    ...
    # Renombrar SP actuales para evitar colisión con target
    for i in range(1, 4):
        df.rename(columns={f"Temperatura SP zona {i}": f"SP_actual zona {i}"}, inplace=True)
    return df
...
feature_cols = [
    ...
    f"SP_actual zona {i}" for i in range(1, 4)
] + [
...
# Target: SP histórica real (ideal a predecir)
...
# Máximos históricos de SP basados en Y_clean
...
tsc, tec, trl = {}, {}, {}
for i in range(1, 4):
    tsc[i] = st.number_input(f"SP_actual zona {i}", min_value=0.0, format="%.1f")
...
if st.button("Calcular SP recomendada"):
    ...
    for i in range(1, 4):
        entrada[f"SP_actual zona {i}"] = tsc[i]
