...
def load_and_train(path="Secadero 1 automático.xlsx"):
    ...
    # Calcular máximos históricos de SP y medias de humedad histórica
    max_sp = {i: int(Y_clean[f"SP_actual zona {i}"].values.max()) for i in range(1, 4)}
    hist_means = {i: df_clean[f"Humedad historica piso {i}"].mean() for i in range(1, 11)}
...
st.write("Introduce los datos actuales; la humedad histórica ya se aplica automáticamente.")
